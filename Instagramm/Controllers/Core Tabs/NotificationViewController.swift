...
final class NotificationViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private lazy var noNotificationViews = NoNotificationsViews()
    ...
    private func addNoNotificationViews() {
        tableView.isHidden = true
        view.addSubview(tableView)
        noNotificationViews.frame = CGRect(x: 0, y: 0, width: view.width / 2, height: view.width / 4)
        noNotificationViews.center = view.center
    }
...
extension NotificationViewController: NotificationLikeEventTableViewCellDelegate {
    func didTapRelatePostButton(model: UserNotification) {
        ...
        case .follow(_):
            fatalError("Dev Issue: Should never get caller")
        }
        // Open the Post
    }
}
